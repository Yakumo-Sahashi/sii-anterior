...
function mostar_area_recorte_dos(url){
    //$('#recorte_img').attr('src', url);
    $('#crear').html('<img src="public/img/' + url +'/fotografia.webp" name="recorte_img" id="recorte_img" alt="Foto Alumno"> ');
    recorte();
}
...
function precargar_foto(alumno,control){
    ver ++;
    $('#input_file').css("display", "none");
    var span = '<span class="btn-cancelar"><a href="#" type="button"><img class="thumb rounded" src="public/img/' + alumno +'/fotografia.webp?ver= ' + ver + '" title="' + control +'"/></a><div class="boton-emergente"><button onclick="limpiar_foto()" class="btn btn-img btn-sm" type="button" title="Eliminar"><i class="fas fa-times" style="font-size: 9px; margin-left: -3px"></i></button></div></span>';
    $('#img_foto').html(span);
    //mostar_area_recorte_dos(alumno);
}